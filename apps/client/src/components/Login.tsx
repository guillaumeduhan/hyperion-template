...
export const OTP: FC<{
  ...
  const { loading, setLoading, fetcher, error, setError } = useHelpers();
...
const Login = () => {
  const { loading, setLoading, error, setError, fetcher } = useHelpers();
  ...
  const signIn = async () => {
    try {
      ...
      const response = await fetcher('/api/auth/login', 'POST', { email });
      console.log(process.env.NODE_ENV);
      console.log(process.env.STAGING_API_URL);
        ...
        {!otp && <div className="grid gap-2">
          ...
          <div className="grid gap-6">
            ...
            {/* <div className="flex flex-col items-center w-full">
              <div className="text-sm text-primary-500 cursor-pointer hover:text-primary-700" onClick={() => setCreate(!create)}>
